...
public class HpBar extends Canvas {
    ...
    public void draw() {
        ...
        if (side == Side.HERO) {
            double lenHp = (GameController.getHeroTurret().getCurrentHP() / GameController.getHeroTurret().getMaxHP()) * (double) width;
        ...
        if (side == Side.ENEMY) {
            double lenHp = (GameController.getEnemyTurret().getCurrentHP() / GameController.getEnemyTurret().getMaxHP()) * (double) width;
            gc.setFill(Color.valueOf("#ff0000"));
